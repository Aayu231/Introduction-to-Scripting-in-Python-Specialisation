...
def build_plot_values(gdpinfo, gdpdata):
    ...
    """
    year_gdp = []
    mini_year = gdpinfo['min_year']
    maxi_year = gdpinfo['max_year']

    for key, value in gdpdata.items():
        if(key.isdigit() and isfloat(value)):
            if(value != '' and int(key) <= maxi_year and int(key) >= mini_year):
                year_gdp.append((int(key), float(value)))
...
def build_plot_dict(gdpinfo, country_list):
    ...
    """
    gdp_dat = read_csv_as_nested_dict(gdpinfo["gdpfile"], 
    ...
    for country in country_list:
        try:
            plot_dict[country] = build_plot_values(gdpinfo, gdp_dat[country])
        except KeyError:
            plot_dict[country] = []
            
        plot_dict[country] = sorted(plot_dict[country], key=lambda x: x[0])
