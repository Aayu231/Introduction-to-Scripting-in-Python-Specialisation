...
##            if(str(year) == key[4]):
##                by_year[key] = value
##        by_year_list.append(by_year)
##    return (by_year_list)
    by_year_list = []

    for eachdict in statistics:
        if eachdict[yearid] == str(year):
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for dit in statistics:
        players_list.append((dit[info['playerid']], formula(info, dit)))
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    sep_year = filter_by_year(batting_stats, year, info['yearid'])
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for dit in statistics:
        dicttostore[dit[playerid]] = {}
        for onefield in fields:
            dicttostore[dit[playerid]][onefield] = 0
    ...
    for key, val in dicttostore.items():
        for dit in statistics:
            if key == dit[playerid]:
                # print(True)
                for stat in fields:
                    dicttostore[key][stat] += int(dit[stat])
    return dicttostore
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    indiv_player_stat = aggregate_by_player_id(batting_stats, info['playerid'], fieldnames)
